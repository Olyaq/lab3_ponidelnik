...
class TestCSVReader:
    ...
    def test_reads_valid_csv(self, sample_csv):
        """CSVReader читает файл и возвращает список словарей."""
        reader = CSVReader()
        rows = reader.read(sample_csv)
        assert isinstance(rows, list)
        assert len(rows) == 3  # все строки, включая плохие — ридер не валидирует
    ...
    def test_empty_csv(self, tmp_path):
        """CSV только с заголовком (без данных) — пустой список."""
...
class TestJSONReader:
    ...
    def test_reads_valid_json(self, sample_json):
        """JSONReader читает файл и возвращает список."""
        reader = JSONReader()
        rows = reader.read(sample_json)
        assert isinstance(rows, list)
        assert len(rows) == 1
